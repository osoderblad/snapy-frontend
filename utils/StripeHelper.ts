...
import { GetToApi, postToApi } from "~/helpers/webhelper";
...
export async function getUserSubscription() {
  ...
  const subscription = await postToApi(`stripe/post/subscription/ByUserId`, {
    id: user.stripeCustomerId,
  });

  if (subscription.length === 0) return null;

  var res = mapStripeSubscriptionToSubscriptionDetails(subscription[0]);
